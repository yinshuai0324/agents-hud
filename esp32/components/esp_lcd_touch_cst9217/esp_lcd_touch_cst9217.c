...
static esp_err_t esp_lcd_touch_cst9217_read_data(esp_lcd_touch_handle_t tp)
{
    ...
    if (data[6] != CST9217_ACK_VALUE) {
        // Local fix: with no finger down there is no fresh frame and the ACK
        // byte does not match. The stock driver returns ESP_ERR_INVALID_RESPONSE,
        // which esp_lvgl_port treats as fatal (ESP_ERROR_CHECK -> abort/reboot).
        // Report "zero touch points" instead.
