...
/* Device id = BT MAC suffix, e.g. "F232". Shown on the dial so the user can
 * target it from the Mac: `agents-hud ble connect F232`. */
...
static void start_advertising(bool connectable);
...
static int gap_event_cb(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        if (event->connect.status == 0) {
            ...
            start_advertising(false); /* stay discoverable for `ble list` */
        } else {
...
static void on_sync(void)
{
    ...
    if (ble_hs_id_copy_addr(s_own_addr_type, addr, NULL) == 0) {
        snprintf(s_device_id, sizeof(s_device_id), "%02X%02X", addr[1], addr[0]);
        snprintf(s_device_name, sizeof(s_device_name), "%s-%s",
                 DEVICE_NAME_BASE, s_device_id);
