...
CST9217 触摸）上的用量表盘。纯 **BLE** 与 Mac 通讯：固件是 NimBLE GATT server
（广播名 `AgentsHUD-XXXX`，XXXX 为蓝牙 MAC 后两字节），Mac 端由
[`daemon/`](daemon/) 的推送进程每 3 秒写入一条压缩 JSON——**无需 WiFi、无需配网**。
...
- 三个页面，**点按屏幕循环**：用量（5h/7d 卡片 + 状态栏）→ Clawd 像素宠物（按烧
  ...
  套餐 / **编号**）
- **编号**即蓝牙身份（如 `F232`），等待连接时状态栏也会显示，供
  `agents-hud ble connect <编号>` 指定
- 侧面 **BOOT 键短按**：屏幕旋转 90°（存 NVS，断电记忆）
- 串口调试命令：`b` 重启进下载模式（免按键烧录）、`r` 普通重启
...
- **黑白条纹**：LVGL 绘制缓冲不能放 PSRAM（无线电收发会抢总线导致 QSPI 刷屏丢带），
  必须放内部 DMA RAM，见 `main/bsp_custom.c`（30 行条带 ×2，因 sw_rotate 需双缓冲）。
...
见 [`daemon/README.md`](daemon/README.md)。
